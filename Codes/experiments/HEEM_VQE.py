...
def main(backend, 
    ...
    def callback( evals, params, energy, extra  ): 
        message = {"evaluation": evals, "energy": energy}
        user_messenger.publish( message, message, None )
...
class VQE(MinimumEigensolver):
    ...
    def __init__(self,
                 ...
                 quantum_instance: Optional[Union[QuantumInstance, BaseBackend, Backend]] = Aer.get_backend('aer_simulator') ) -> None:
        ...
        if initial_point is None and hasattr(ansatz, 'preferred_init_points'):
            ...
            self._initial_point = 0.1*np.ones(self._ansatz.num_parameters)
    ...
    def compute_minimum_eigenvalue(self, operator: OperatorBase, aux_operators: Optional[
        ...
        fun = lambda params : self._energy_evaluation( params )
                
        results = self._optimizer.optimize( self._ansatz.num_parameters ,
                                 fun , 
                                 self._initial_point )
