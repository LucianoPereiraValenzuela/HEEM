...
optimizer = COBYLA(maxiter=100, tol=1e-4, disp=True)  # Set up classical optimizer for VQE
...
bounds = [[-2 * np.pi, 2 * np.pi]] * num_vars
...
energy_list = []  # List with the energies of each iteration of the VQE algorithm
result = minimize(objective_function, initial_params, options={'maxiter': 100, 'disp': True}, constraints=cons,
                  method='COBYLA')
