...
def molecule_potentials_comparison(distances, quantum_instance, conectivity, file_name_out):
    ...
    for i, d in enumerate(distances):
        ...
        # Exact energy
        E_EXACT[i] = NumPyMinimumEigensolver().compute_minimum_eigenvalue(qubit_op).eigenvalue.real 
        ...
        solver_TPB  = VQE( ansatz , optimizer, initial_params, grouping = 'TPB', quantum_instance = quantum_instance, callback=callback )
        solver_ENT  = VQE( ansatz , optimizer, initial_params, grouping = 'Entangled', quantum_instance = quantum_instance, callback=callback )
        solver_HEEM = VQE( ansatz , optimizer, initial_params, grouping = 'Entangled', conectivity = conectivity, quantum_instance = quantum_instance, callback=callback )
        
        print('TPB',i)
        E_TPB[i]  = solver_TPB.compute_minimum_eigenvalue(qubit_op).eigenvalue.real
        print('ENT',i)
        E_ENT[i]  = solver_ENT.compute_minimum_eigenvalue(qubit_op).eigenvalue.real
        print('HEEM',i)
        E_HEEM[i] = solver_HEEM.compute_minimum_eigenvalue(qubit_op).eigenvalue.real
    
    # Save results
    np.savez(file_name_out, distances = distances, E_EXACT = E_EXACT, E_TPB = E_TPB, 
             E_ENT = E_ENT, E_HEEM = E_HEEM) 
...
NUM_SHOTS     = 2  # Number of shots for each circuit
...
E = molecule(1.15, qi, conectivity)