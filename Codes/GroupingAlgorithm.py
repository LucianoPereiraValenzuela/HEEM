...
Comp = [[[]], [[0], [1]], [[0], [2]], [[0], [3]], [[0, 0], [1, 1], [2, 2], [3, 3]], [[0, 0], [1, 1], [2, 3], [3, 2]],
        [[0, 0], [2, 2], [1, 3], [3, 1]], [[0, 0], [3, 3], [1, 2], [2, 1]], [[0, 0], [1, 2], [2, 3], [3, 1]],
        [[0, 0], [2, 1], [3, 2], [1, 3]]]
...
def PauliGraph(PS, print_progress=False):
    ...
    ------
    PG: graph
        The Pauli graph represents the non commutativity of the n Pauli strings.
    ...
    pbar = None
    if print_progress:
        pbar = tqdm(total=n, desc='Computing Pauli graph')
...
def TPBgrouping(PS, print_progress=False):
    """
    ...
    PG = PauliGraph(PS, print_progress=print_progress)
...
def MeasurementAssignment(Vi, Vj, Mi, AM, WC):
    ...
    # and we begin again if U is not empty. If we managed to empty U, the update would have succeeded, and we would
    # return UMi, S=True. If there is no success, Mi, S=False are returned.
...
def number_of_compatible_measurements_onequbit(m, F):
    """
    Given a measurement 'm' and an array of one-qubit factors 'F', calculates the number of compatible measurements
    that can be made with that measurement in those factors
...
def Tcompatiblities(PS, T, G):  # Algorithm 4 of Fran's notes.
    ...
    G: graph
        G is the connectivity graph of the chip. Its vertices represent physical qubits and its edges physical
...
def groupingWithOrder(PS, G=None, connected=False, print_progress=False):
    ...
    connected: boolean
        ...
        chip is connected. If connected=False the transpile_HEEM algorithm does not ensure that the subgraph of
    ...
    pbar = None
    if print_progress:
        pbar = tqdm(total=n, desc='Computing grouping')
